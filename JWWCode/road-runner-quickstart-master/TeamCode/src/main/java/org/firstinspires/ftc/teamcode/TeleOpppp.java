...
@TeleOp(name = "FineTuneTeleOp", group = "TeleOp")
public class TeleOpppp extends OpMode {
    ...
    public void loop() {
        ...
        if (gamepad1.left_bumper) {
            changeServoPositionBy(clawServoBottom, .0002);
        ...
        if (gamepad2.right_trigger > 0) {
            changeServoPositionBy(wristLeft, .0002);
            changeServoPositionBy(wristRight, -.0002);
        ...
        if (gamepad2.right_bumper) {
            changeServoPositionBy(clawServoTop, .0002);
        } else if(gamepad2.left_bumper) {
            changeServoPositionBy(clawServoTop, -.0002);
        }
        // claw wrist
        if (gamepad2.dpad_up) {
            changeServoPositionBy(clawWrist, .0002);
        ...
        // shoulder
        if (gamepad2.y) {
            changeServoPositionBy(shoulderLeft, .0002);
...
//        }
    ...
    private void updateAllTelemetry() {
        ...
        telemetry.addData("Top pivot: ", topPivot.getPosition());
        telemetry.addData("Bottom pivot: ", bottomPivot.getPosition());
        telemetry.update();
    }
